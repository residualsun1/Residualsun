---
title: 利用 Git LFS 上传大文件到 GitHub 中
author: 黄国政
date: '2025-05-09'
slug: git-lfs
categories: []
...
建立网页最便捷的一个方式是利用 GitHub 的 Pages 页面进行部署。

一般来说，在本地路径建立一个包含 `docs` 文件夹的父文件夹，其中，`docs` 文件夹内必须包含 `index.html` 文件，之后将该父文件夹推送至对应的 GitHub 远程仓库中，之后在 Pages 页面中将 Branch 处调整为 `main` 和 `/docs`，GitHub 即可部署形成网页，默认网址为 `https://用户名.github.io/仓库名`。
...
由此，本期博文的主题无关网页内容，而是**如何利用 Git 将大文件上传到 GitHub**。
...
不知道什么原因，前面已经成功通过 LFS 成功推送大文件，当推送常规文件时，还是显示大文件。
...
```bash
$ git lfs ls-files
8607a53a2f * docs/Data/cfps2022person_202410.dta
```

根据 Deepseek 的说法，原因是大文件在 Git 的历史提交中仍存在未被 LFS 管理的版本。GitHub 会扫描所有历史记录，若发现任何提交包含大文件（即使后来被 LFS 管理），也会拒绝推送。
