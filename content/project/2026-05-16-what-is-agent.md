...
<center>（1）环境（2）传感器（3）执行器（4）自主（5）行动（6）实体</center>
...
感知与获取到环境中的信息后，智能体会无需人为干预地「自主」通过「执行器」来采取行动，进而改变环境状态。这些「执行器」并不复杂，它可能是物理设备上的方向盘、机械臂，也可能是虚拟工具中的一段代码。

值得注意的是，这里的「自主」需要做一个区分——究竟是依据预先设定好的程序或脚本来自主运行，还是根据模型的推理来独立自主地进行决策？

不同类型的「自主」是划分传统智能体与现代智能体的重要指标，接下来我们会在对智能体发展历史的梳理中认识不同类型的智能体。
...
最后让我们回到智能体（Agent）。虽然在文章的开头我们就给智能体下了一个定义，但在这里不妨根据其与 AI、LLM、API 的关系，再做一次定义。

在我看来，智能体可以被理解为在 AI 领域中基于 LLM 构建的应用形态，它可以通过 API 调用各种工具（包括 LLM），在过去 LLM 只能进行对话的基础上迈进一步，还可以执行任务，如读写文件、操作数据库等等。

概而言之，智能体就是一种具备能力的实体，不再仅仅只是执行预设程序的脚本，而是能够自主推理和使用工具的决策者。

## Agent 的应用实例

在实际的应用场景中，Agent 表现出两种协作模式，一种是深度融入开发工作流的高效工具，另一种是自主协作者。

### 一、融入开发者工具的 Agent

当下有许多智能体被深度集成到开发者工作流中的实例，近来涌现的 AI 编程辅助工具就是很好的体现，如 CitHubCopilot、Claude Code、Trae 和 Cursor。

|               | 定位          | 特点                                                      |   |
|---------------|-------------|---------------------------------------------------------|---|
| GitHubCopilot | 深度集成于主流编辑器中 | 可实时提供整行甚至整个函数块的建议                                       |   |
| Claude Code   | AI 编程助手     | 可在终端中通过自然语言完成任务，理解完整代码库结构，执行代码编辑，测试和调试等操作               |   |
| Trae          | AI 编程工具     | 提供智能化代码生成和优化服务                                          |   |
| Cursor        | AI 原生的代码编辑器 | 不是在编辑器上增加 AI 功能，而是设计之初就将与 AI 的交互作为核心。强调让 AI 理解整个代码库上下文。 |   |

### 二、自主协作的 Agent

融入开发者工具的 Agent 的定位在于辅助人类进行开发，需要开发者手把手地指导 AI 完成每一步任务。但自主协作的方向将 Agent 的自主程度提高了全新层次，它会作为一个真正的项目成员独立地进行规划、推理、执行和反思，直到最后交付成果。我们与 AI 的关系也从「命令-执行」转变为「目标-委托」

自主协作式的 Agent 产品或框架有 BabyAGI、AutoGPT、CrewAI、AutoGen、MetaGPT、LangGraph 等。让我们来看看它们的架构范式：

#### 2.1 单智能体自主循环

早期的典型范式，以 AgentGPT 为代表，核心是一个通用智能体通过「思考-规划-执行-反思」的闭环不断进行自我提示和迭代，以完成开放式的高层次目标。
...
根据我们上一篇文章中的实战结果，可以总结一下 Agent 的核心价值：**基于实时信息进行动态推理和决策的能力**。

在我们向 Agent 发出 `你好，请帮我查询一下今天北京的天气，然后根据天气推荐一个合适的旅游景点。` 这样的指令后，Agent 并非如传统工作流（workflow）一样只是对一系列任务或步骤进行预先定义的、结构化的编排，而是在一定程度上理解环境、进行推理，进而制定计划，并动态地采取行动进行完成目标：

* **规划与调用工具**：将 `你好，请帮我查询一下今天北京的天气，然后根据天气推荐一个合适的旅游景点。` 这一任务拆分为两个步骤——1、查询北京天气；2、基于北京天气推荐景点。接下来，Agent 会自主选择和调用天气查询的 API，并将 `北京` 作为参数传入。
* **推理与决策**：如果 API 返回的结果是 `sunny`，Agent 中的 LLM （大脑）就会基于这一信息进行推理——晴朗的天气适合户外活动，进而根据这一判断在知识库或搜索引擎这些工具中筛选北京的户外景点。
